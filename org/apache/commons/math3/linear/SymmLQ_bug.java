...
public class SymmLQ_bug
    ...
    private static class State {
        ...
        private static void checkSymmetry(final RealLinearOperator l,
            ...
            if (FastMath.abs(s - t) > epsa) {
                ...
                context.setValue(SymmLQ_bug.OPERATOR, l);
                context.setValue(SymmLQ_bug.VECTOR1, x);
                context.setValue(SymmLQ_bug.VECTOR2, y);
                context.setValue(SymmLQ_bug.THRESHOLD, Double.valueOf(epsa));
                throw e;
            }
        }
     ...
     * preconditioner should be checked
     */
    public SymmLQ_bug(final int maxIterations, final double delta,
                  final boolean check) {
        super(maxIterations);
        this.delta = delta;
        this.check = check;
     ...
     * preconditioner should be checked
     */
    public SymmLQ_bug(final IterationManager manager, final double delta,
                  final boolean check) {
        super(manager);
        this.delta = delta;
        this.check = check;
